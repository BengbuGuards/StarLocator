 ...
 * @returns {Date} 返回对应时间
 */
async function calc(stars, z, zenith, approxDate, scopeDate, astroCalculator, isFixGravity = false, isFixRefraction = false) {
    let moonIndex = findMoonIndex(stars);
    // 获取月与各星相互角距作为目标值
    let targetAngles = AngleBetweenMoonAndStar(stars, moonIndex, z);
    // 根据星星信息计算大致日期下的地理坐标
    let geoEstimate = await geoEstimatebyStars(approxDate, stars, z, zenith, astroCalculator, moonIndex, isFixGravity, isFixRefraction);
