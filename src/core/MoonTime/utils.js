...
function findMoonIndex(stars) {
    let moonIndex = stars.findIndex(
        (star) =>
            star.name === '月' || star.name === '月球' || star.name === '月亮' || star.name.toLowerCase() === 'moon'
    );
    return moonIndex;
}
 ...
 * @returns {Date} 返回对应时间
 */
async function geoEstimatebyStars(
    approxDate,
    stars,
    z,
    zenith,
    ...
    let approxStarHaDecs = await astroCalculator.getHaDecbyNames(
...
async function angleError(time, approxDate, stars, geoEstimate, moonIndex, astroCalculator, targetAngles) {
    ...
    // 获取该时间、该地理坐标下的天体时角赤纬
    let starHaDecs = await astroCalculator.getHaDecbyNames(
        stars.map((star) => star.name),
        date,
        observer
    );
