...
import { AngleBetweenMoonAndStar, geoEstimatebyStars, angleError } from "./utils.js";
 ...
 * @returns {Date} 返回对应时间
 */
async function calc(stars, z, zeniths, approxDate, scopeDate, astroCalculator, isFixGravity = false, isFixRefraction = false) {
    ...
    // 根据星星信息计算大致日期下的地理坐标
    let geoEstimate = await geoEstimatebyStars(approxDate, stars, z, zeniths, astroCalculator, moonIndex, isFixGravity, isFixRefraction);
    ...
    let optTime = 0;
    let optFunc = async (time) => await angleError(time, approxDate, stars, geoEstimate, moonIndex, astroCalculator, targetAngles);
